                      ...
                      itemBuilder: (context, index) {
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: TextField(
                                      ...
                                      decoration: const InputDecoration(hintText: 'Search marts by name or address', border: InputBorder.none, isDense: true),
                                      onChanged: (_) => setState(() {}),
                                    ),
                                  ),
                                  if (_searchController.text.isNotEmpty)
                                    IconButton(onPressed: () => setState(() => _searchController.clear()), icon: const Icon(Icons.clear, color: Colors.black45))
                                  else
                                    IconButton(onPressed: () => ref.read(martViewModelProvider.notifier).fetchAllMarts(), icon: const Icon(Icons.refresh, color: Colors.black54)),
                                ],
                              ),
                            ),
                          );
                        ...
                        if (marts.isEmpty) {
                          return Column(
                            children: const [
                              SizedBox(height: 40),
                              Center(child: Icon(Icons.storefront, size: 56, color: Colors.black12)),
                              SizedBox(height: 12),
                              Center(child: Text('No marts yet', style: TextStyle(color: Colors.black54, fontSize: 16))),
                            ],
                          );
                        }
                        ...
                        return Dismissible(
                          ...
                          direction: mart.id != null ? DismissDirection.endToStart : DismissDirection.none,
                          ...
                          confirmDismiss: (direction) async {
                            ...
                            final confirmed = await showDialog<bool>(
                              ...
                              builder: (_) => AlertDialog(
                                ...
                                content: Text('Delete "${mart.name}"? This action cannot be undone.'),
                                actions: [
                            ...
                            final success = await ref.read(martViewModelProvider.notifier).deleteMart(mart.id!, context);
                            return success;
                          },
                          child: Container(
                            ...
                            child: InkWell(
                              borderRadius: BorderRadius.circular(12),
                              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MartDetailView(mart: mart))),
                              child: Padding(
                                ...
                                child: Row(
                                  children: [
                                    CircleAvatar(
                                      ...
                                      child: Text((mart.name.isNotEmpty ? mart.name[0].toUpperCase() : '?'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.black87)),
                                    ),
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text(mart.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                                          const SizedBox(height: 6),
                                          Text(mart.address ?? mart.description ?? '', style: const TextStyle(color: Colors.black54), maxLines: 2, overflow: TextOverflow.ellipsis),
                                          const SizedBox(height: 6),
                                          Row(children: [
                                            if ((mart.contactEmail ?? '').isNotEmpty) Text(mart.contactEmail ?? '', style: const TextStyle(fontSize: 12, color: Colors.black45)),
                                            const Spacer(),
                                            Text(mart.isActive == true ? 'Active' : 'Inactive', style: TextStyle(color: mart.isActive == true ? Colors.green : Colors.red, fontSize: 12)),
                                          ])
                                        ],
                                      ),
                                    ),
